...
def build_retry_session(config: Dict) -> requests.Session:
    ...
    session.headers.update({
        "User-Agent": "exoplanet-archive-ambience-short/1.0 (educational visualization)"
    })
    return session
...
def build_exoplanet_query() -> str:
    ...
    - Quote "dec" defensively because ADQL supports double-quoted identifiers
      for keyword/name conflicts. The returned CSV column is still named dec.
    - Do not request rowupdate here; it can cause HTTP 400 errors on PSCompPars.
    """
    query_fields = [
        "pl_name",
        "hostname",
