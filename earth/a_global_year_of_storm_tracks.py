...
across the globe through one calendar year. Tracks appear chronologically,
then resolve into basin-by-basin highlights, strongest-storm spotlights, and a
final global "fingerprint" of the year.
...
The current UTC calendar year is used by default. For a completed year:
    ...
    STORM_TRACK_YEAR=2025 python a_global_year_of_storm_tracks.py
...
--------------------
    STORM_TRACK_SHORT_QUICK=1 python a_global_year_of_storm_tracks.py

Force offline fixture mode
--------------------------
    STORM_TRACK_SHORT_OFFLINE=1 python a_global_year_of_storm_tracks.py
...
QUICK_MODE = os.environ.get("STORM_TRACK_SHORT_QUICK", "0") == "1"
OFFLINE_MODE = os.environ.get("STORM_TRACK_SHORT_OFFLINE", "0") == "1"


def resolve_year_window() -> Tuple[datetime, datetime, int, bool]:
    now = datetime.now(timezone.utc)
    override = os.environ.get("STORM_TRACK_YEAR", "").strip()
    if override:
        try:
            year = int(override)
        except ValueError as exc:
            raise ValueError("STORM_TRACK_YEAR must be a four-digit year, for example 2025") from exc
        if year < 1840 or year > now.year:
            raise ValueError(f"STORM_TRACK_YEAR must be between 1840 and {now.year}")
    else:
        year = now.year
...
OUTPUT_ROOT = Path(f"a_global_year_of_storm_tracks_{YEAR_KEY}_output")
...
CONFIG: Dict[str, Any] = {
    ...
    "output_basename": f"a_global_year_of_storm_tracks_{YEAR_KEY}",
    "title": "A GLOBAL YEAR OF STORM TRACKS",
    "subtitle": f"{YEAR_LABEL} // NOAA IBTRACS V4R01",
...
def request_bytes(url: str, timeout: int = 60) -> bytes:
    request = urllib.request.Request(
        url,
        headers={
            "User-Agent": "Mozilla/5.0 (compatible; GlobalStormTracksShort/1.0; educational visualization)",
...
def load_storms() -> Tuple[List[Storm], str, List[str], Optional[Path]]:
    ...
    if OFFLINE_MODE:
        notes.append("Offline mode requested with STORM_TRACK_SHORT_OFFLINE=1")
...
class StormTrackScene:
    ...
    def draw_opening(self, image: Image.Image, t: float, shot: Dict[str, Any]):
        ...
        map_layer = self.compose_map(None, opacity=0.35 + 0.55 * p)
        ...
        sd.rectangle((x - 2, CONFIG["map_top"], x + 2, CONFIG["map_bottom"]), fill=COLORS["ts"] + (120,))
        scan = scan.filter(ImageFilter.GaussianBlur(max(1, int(8 * SCALE))))
        ...
        draw_text(
            ...
            (OUT_W // 2, int(OUT_H * 0.83)),
            15 if QUICK_MODE else 30,
            COLORS["muted"] + (235,),
            bold=True,
            condensed=True,
            anchor="ma",
    ...
    def draw_year_sweep(self, image: Image.Image, t: float, shot: Dict[str, Any]):
        p = smootherstep(shot_progress(t, shot))
        bucket = min(int(p * len(self.timeline_layers)), len(self.timeline_layers) - 1)
        layer = self.compose_map(self.timeline_layers[bucket])
        image.alpha_composite(layer)

        current_time = YEAR_START + (YEAR_END - YEAR_START) * p
        count = int(np.searchsorted(self.start_fractions, p, side="right"))

        # Active-position flashes around the current date.
        overlay = Image.new("RGBA", OUT_SIZE, (0, 0, 0, 0))
        od = ImageDraw.Draw(overlay)
        window_seconds = max((YEAR_END - YEAR_START).total_seconds() * 0.025, 3 * 86400)
        for storm in self.storms:
            nearest = min(storm.points, key=lambda pt: abs((pt.time_utc - current_time).total_seconds()))
            dt = abs((nearest.time_utc - current_time).total_seconds())
            if dt > window_seconds:
                continue
            x, y = self.project(nearest.longitude, nearest.latitude)
            r = max(2.5, 5.0 * SCALE)
            color = intensity_color(nearest.wind_kt, 245)
            od.ellipse((x - r, y - r, x + r, y + r), fill=color)
            od.ellipse((x - r * 2.4, y - r * 2.4, x + r * 2.4, y + r * 2.4), outline=color[:3] + (110,), width=max(1, int(2 * SCALE)))
        ...
        pd.rounded_rectangle(
            (left, top, OUT_W - left, top + int(245 * SCALE)),
            radius=max(12, int(26 * SCALE)),
            fill=(2, 7, 14, 185),
            outline=COLORS["grid"] + (70,),
            width=1,
        )
        image.alpha_composite(panel)
        draw_text(image, current_time.strftime("%d %B").upper(), (left + int(30 * SCALE), top + int(36 * SCALE)), 25 if not QUICK_MODE else 13, COLORS["white"] + (250,), bold=True, condensed=True, stroke=1)
        draw_text(image, f"{count:,}", (left + int(30 * SCALE), top + int(145 * SCALE)), 54 if not QUICK_MODE else 27, COLORS["white"] + (255,), bold=True, condensed=True, stroke=2)
        draw_text(image, "STORMS HAVE ENTERED THE YEAR", (left + int(170 * SCALE), top + int(155 * SCALE)), 18 if not QUICK_MODE else 9, COLORS["muted"] + (235,), bold=True, condensed=True, stroke=1)

    def draw_basin_relay(self, image: Image.Image, t: float, shot: Dict[str, Any]):
        ...
        basin = available[idx]
        local = (p * len(available)) - idx
        base = self.compose_map(None, opacity=0.72)
        image.alpha_composite(base)
        tracks = self.basin_layers[basin]
        alpha = smoothstep(min(local * 3.0, 1.0)) * smoothstep(min((1.0 - local) * 3.0 + 0.15, 1.0))
        if alpha < 0.15:
            alpha = 0.15
        temp = tracks.copy()
        a = temp.getchannel("A").point(lambda v: int(v * alpha))
        temp.putalpha(a)
        image.alpha_composite(temp)

        count = sum(1 for s in self.storms if s.basin == basin)
        draw_text(image, BASIN_NAMES.get(basin, basin), (OUT_W // 2, int(OUT_H * 0.76)), 30 if not QUICK_MODE else 15, COLORS["white"] + (255,), bold=True, condensed=True, anchor="ma", stroke=2)
        draw_text(image, f"{count} TRACK{'S' if count != 1 else ''}", (OUT_W // 2, int(OUT_H * 0.82)), 20 if not QUICK_MODE else 10, COLORS["muted"] + (235,), bold=True, condensed=True, anchor="ma", stroke=1)
    ...
    def draw_strongest(self, image: Image.Image, t: float, shot: Dict[str, Any]):
        ...
        base = self.compose_map(self.all_tracks, opacity=0.45)
        image.alpha_composite(base)
        highlight = self._render_tracks([storm], 1.0, alpha_scale=1.0)
        ...
        rr = (15 + 8 * math.sin(t * 5.0) ** 2) * SCALE
        color = intensity_color(peak.wind_kt, 255)
        pd.ellipse((x - rr, y - rr, x + rr, y + rr), outline=color, width=max(1, int(3 * SCALE)))
        pulse = pulse.filter(ImageFilter.GaussianBlur(max(1, int(1.5 * SCALE))))
        image.alpha_composite(pulse)

        wind_text = "WIND N/A" if not np.isfinite(storm.max_wind_kt) else f"{storm.max_wind_kt:.0f} KT"
        mph_text = "" if not np.isfinite(storm.max_wind_kt) else f"  //  {storm.max_wind_kt * 1.15078:.0f} MPH"
        draw_text(image, storm.display_name, (int(58 * SCALE), int(1500 * SCALE)), 38 if not QUICK_MODE else 19, COLORS["white"] + (255,), bold=True, condensed=True, stroke=2)
        draw_text(image, f"{BASIN_NAMES.get(storm.basin, storm.basin)}  //  {wind_text}{mph_text}", (int(58 * SCALE), int(1585 * SCALE)), 18 if not QUICK_MODE else 9, color[:3] + (245,), bold=True, condensed=True, stroke=1)
        draw_text(image, intensity_label(storm.max_wind_kt), (int(58 * SCALE), int(1640 * SCALE)), 16 if not QUICK_MODE else 8, COLORS["muted"] + (230,), bold=True, condensed=True, stroke=1)

    def draw_global_pattern(self, image: Image.Image, t: float, shot: Dict[str, Any]):
        p = smoothstep(shot_progress(t, shot))
        image.alpha_composite(self.compose_map(self.all_tracks))
        overlay = Image.new("RGBA", OUT_SIZE, (0, 0, 0, 0))
        od = ImageDraw.Draw(overlay)
        y = int(1490 * SCALE)
        od.rounded_rectangle((int(45 * SCALE), y, OUT_W - int(45 * SCALE), y + int(260 * SCALE)), radius=max(12, int(28 * SCALE)), fill=(2, 7, 14, int(175 + 30 * p)), outline=COLORS["grid"] + (70,), width=1)
        image.alpha_composite(overlay)
        draw_text(image, f"{len(self.storms):,}", (OUT_W // 2, y + int(86 * SCALE)), 76 if not QUICK_MODE else 38, COLORS["white"] + (255,), bold=True, condensed=True, anchor="ma", stroke=2)
        draw_text(image, "GLOBAL STORM TRACKS", (OUT_W // 2, y + int(172 * SCALE)), 24 if not QUICK_MODE else 12, COLORS["muted"] + (240,), bold=True, condensed=True, anchor="ma", stroke=1)

    def draw_finale(self, image: Image.Image, t: float, shot: Dict[str, Any]):
        p = smoothstep(shot_progress(t, shot))
        layer = self.compose_map(self.all_tracks)
        image.alpha_composite(layer)
        veil = Image.new("RGBA", OUT_SIZE, (0, 0, 0, int(55 * p)))
        image.alpha_composite(veil)
        draw_text(image, "ONE PLANET", (OUT_W // 2, int(OUT_H * 0.72)), 34 if not QUICK_MODE else 17, COLORS["white"] + (255,), bold=True, condensed=True, anchor="ma", stroke=2)
        draw_text(image, "ONE YEAR", (OUT_W // 2, int(OUT_H * 0.78)), 34 if not QUICK_MODE else 17, COLORS["white"] + (255,), bold=True, condensed=True, anchor="ma", stroke=2)
        draw_text(image, "EVERY TRACK IN THE SELECTED ARCHIVE", (OUT_W // 2, int(OUT_H * 0.85)), 18 if not QUICK_MODE else 9, COLORS["muted"] + (240,), bold=True, condensed=True, anchor="ma", stroke=1)
    ...
    def draw_header(self, image: Image.Image, name: str):
        if name == "opening":
            ...
            title_alpha = 245
        if title_alpha:
            draw_text(image, "A GLOBAL YEAR", (int(48 * SCALE), int(72 * SCALE)), 22 if not QUICK_MODE else 11, COLORS["white"] + (title_alpha,), bold=True, condensed=True, stroke=1)
            draw_text(image, "OF STORM TRACKS", (int(48 * SCALE), int(118 * SCALE)), 38 if not QUICK_MODE else 19, COLORS["white"] + (title_alpha,), bold=True, condensed=True, stroke=2)
            draw_text(image, CONFIG["subtitle"], (int(50 * SCALE), int(170 * SCALE)), 14 if not QUICK_MODE else 7, COLORS["muted"] + (220,), bold=True, condensed=True, stroke=1)
    ...
    def draw_legend(self, image: Image.Image, name: str):
        if name not in {"year_sweep", "global_pattern", "finale"}:
            return
        items = [("TS", 45), ("64+ KT", 70), ("96+ KT", 100), ("113+ KT", 125)]
        x = int(55 * SCALE)
        y = int(1395 * SCALE)
        for label, wind in items:
            ...
            draw_text(image, label, (x + int(16 * SCALE), y), 12 if not QUICK_MODE else 6, COLORS["muted"] + (220,), bold=True, condensed=True, anchor="lm", stroke=1)
            x += int((130 if not QUICK_MODE else 65) * SCALE * (2 if QUICK_MODE else 1)) if False else int(220 * SCALE)
    ...
    def render_frame(self, t: float) -> np.ndarray:
        ...
        elif name == "year_sweep":
            self.draw_year_sweep(image, t, shot)
        elif name == "basin_relay":
            self.draw_basin_relay(image, t, shot)
        elif name == "strongest":
            self.draw_strongest(image, t, shot)
        elif name == "global_pattern":
            self.draw_global_pattern(image, t, shot)
        else:
...
def generate_ambient_soundtrack(path: Path) -> Path:
    ...
    sweep = next(s for s in SHOT_PLAN if s["name"] == "year_sweep")
    ...
    # Scene-impact swells.
    for scene_name, strength in [("basin_relay", 0.08), ("strongest", 0.12), ("global_pattern", 0.10), ("finale", 0.14)]:
...
def render_video(scene: StormTrackScene) -> Path:
    ...
    with iio.get_writer(
        ...
        for t in tqdm(times, desc="Rendering global storm-track short"):
