...
schematic. This is NOT a live eruption feed, earthquake forecast, or formal GIS
plate-motion product.

No internet connection or external data is required. All animation is
deterministic.
...
--------------------
    RING_OF_FIRE_SHORT_QUICK=1 python the_ring_of_fire_is_always_moving_short.py

Full render
-----------
    python the_ring_of_fire_is_always_moving_short.py
...
QUICK_MODE = os.environ.get("RING_OF_FIRE_SHORT_QUICK", "0") == "1"

OUTPUT_ROOT = Path("the_ring_of_fire_is_always_moving_output")
...
class RingOfFireScene:
    def __init__(self):
        ...
        self.volcanoes = [
            ...
            for name, lat, lon, setting in VOLCANOES[:47]
        ]
    ...
    def draw_motion_vectors(self, image: Image.Image, t: float, alpha: int = 220):
        """Draw schematic relative-motion arrows around the Pacific margin."""
        overlay = Image.new("RGBA", OUT_SIZE, (0, 0, 0, 0))
        draw = ImageDraw.Draw(overlay)
        # lon, lat, dx_px, dy_px, label. Directions are schematic for storytelling.
        vectors = [
            (-150, 46, 52, 8, "PACIFIC"),
            (-126, 18, 34, 18, "COCOS"),
            (-84, -24, -30, 16, "NAZCA"),
            (158, 44, -35, 14, "PACIFIC"),
            (143, 20, -30, 12, "PHILIPPINE SEA"),
            (165, -30, -28, -10, "PACIFIC"),
        ]
        pulse = 0.82 + 0.18 * math.sin(t * 2.2)
        for lon, lat, dx, dy, label in vectors:
            x, y = self.project(lon, lat)
            dx *= SCALE * pulse
            dy *= SCALE * pulse
            x2, y2 = x + dx, y + dy
            draw.line((x, y, x2, y2), fill=COLORS["cyan"] + (alpha,), width=max(1, int(4*SCALE)))
            ang = math.atan2(y2-y, x2-x)
            ah = 14*SCALE
            a1 = ang + math.pi*0.82
            a2 = ang - math.pi*0.82
            draw.polygon([(x2,y2), (x2+ah*math.cos(a1), y2+ah*math.sin(a1)), (x2+ah*math.cos(a2), y2+ah*math.sin(a2))], fill=COLORS["cyan"] + (alpha,))
            draw_text(overlay, label, (int(x), int(y-15*SCALE)), size=10 if not QUICK_MODE else 5, fill=COLORS["white"] + (170,), bold=True, anchor="ma", stroke=1)
        image.alpha_composite(overlay)
    ...
    def draw_pattern_scene(self, image: Image.Image, t: float):
        ...
        self.panel(image, (int(OUT_W*0.07), int(OUT_H*0.675), int(OUT_W*0.93), int(OUT_H*0.82)), alpha=180)
        draw_text(image, "A HORSESHOE OF TECTONIC ACTIVITY", (OUT_W//2, int(OUT_H*0.718)), size=24 if not QUICK_MODE else 12, fill=COLORS["gold"]+(245,), bold=True, anchor="ma", stroke=1)
        draw_text(image, "volcanoes + trenches + earthquake belts around the Pacific", (OUT_W//2, int(OUT_H*0.765)), size=16 if not QUICK_MODE else 8, fill=COLORS["white"]+(225,), anchor="ma", stroke=1)

    def draw_boundaries_scene(self, image: Image.Image, t: float):
        shot = next(item for item in SHOT_PLAN if item["name"] == "boundaries")
        local = smoothstep((t - shot["start"]) / max(shot["end"] - shot["start"], 1e-9))
        self.draw_world_map(image, grid_alpha=22)
        self.draw_boundaries(image, reveal=min(1.0, local*1.25))
        self.draw_ring_of_fire(image, 0.65 + 0.35*local)
        self.draw_motion_vectors(image, t, alpha=int(150 + 80*local))
        self.draw_volcanoes(image, reveal=1.0, pulse=t, alpha=205)
        self.panel(image, (int(OUT_W*0.07), int(OUT_H*0.675), int(OUT_W*0.93), int(OUT_H*0.83)), alpha=184)
        draw_text(image, "THE PLATES ARE MOVING", (OUT_W//2, int(OUT_H*0.716)), size=27 if not QUICK_MODE else 13, fill=COLORS["cyan"]+(248,), bold=True, anchor="ma", stroke=1)
        draw_text(image, "relative motion is typically centimeters per year", (OUT_W//2, int(OUT_H*0.762)), size=16 if not QUICK_MODE else 8, fill=COLORS["white"]+(225,), anchor="ma", stroke=1)
        draw_text(image, "arrows are schematic — not live GPS vectors", (OUT_W//2, int(OUT_H*0.798)), size=13 if not QUICK_MODE else 6, fill=COLORS["muted"]+(185,), anchor="ma", stroke=1)
    ...
    def draw_subduction_scene(self, image: Image.Image, t: float):
        ...
        draw_text(image, "SUBDUCTION KEEPS THE MARGIN MOVING", (OUT_W // 2, int(OUT_H * 0.728)), size=24 if not QUICK_MODE else 12, fill=COLORS["lava"] + (245,), bold=True, anchor="ma", stroke=1)
        draw_text(image, "a plate bends into a trench, descends, and recycles into the mantle", (OUT_W // 2, int(OUT_H * 0.775)), size=16 if not QUICK_MODE else 8, fill=COLORS["white"] + (225,), anchor="ma", stroke=1)

    def draw_rift_scene(self, image: Image.Image, t: float):
        shot = next(item for item in SHOT_PLAN if item["name"] == "rift")
        local = smoothstep((t - shot["start"]) / max(shot["end"] - shot["start"], 1e-9))
        top, bottom = int(OUT_H*0.22), int(OUT_H*0.63)
        left, right = int(OUT_W*0.07), int(OUT_W*0.93)
        draw = ImageDraw.Draw(image)
        draw.rounded_rectangle((left, top, right, bottom), radius=max(10,int(24*SCALE)), fill=(8,21,34,240), outline=COLORS["cyan"]+(48,), width=1)
        fault_x = OUT_W*0.50
        gap = 8*SCALE
        # Two crustal blocks sliding past each other along a transform fault.
        draw.rectangle((left, top+80*SCALE, fault_x-gap, bottom-25*SCALE), fill=COLORS["crust"]+(255,), outline=COLORS["land_light"]+(210,))
        draw.rectangle((fault_x+gap, top+80*SCALE, right, bottom-25*SCALE), fill=COLORS["crust"]+(255,), outline=COLORS["land_light"]+(210,))
        draw.line((fault_x, top+45*SCALE, fault_x, bottom-10*SCALE), fill=COLORS["red"]+(235,), width=max(2,int(6*SCALE)))
        offset = 26*SCALE*(0.5-0.5*math.cos(local*math.pi))
        # Road-like markers reveal opposite motion.
        for i in range(6):
            y = top + (120+i*42)*SCALE
            draw.line((left+35*SCALE, y-offset, fault_x-22*SCALE, y-offset), fill=COLORS["gold"]+(170,), width=max(1,int(3*SCALE)))
            draw.line((fault_x+22*SCALE, y+offset, right-35*SCALE, y+offset), fill=COLORS["gold"]+(170,), width=max(1,int(3*SCALE)))
        # Opposing arrows parallel to the boundary.
        ax = fault_x-70*SCALE; bx = fault_x+70*SCALE
        draw.line((ax, top+45*SCALE, ax, top+130*SCALE), fill=COLORS["cyan"]+(235,), width=max(1,int(4*SCALE)))
        draw.polygon([(ax, top+130*SCALE),(ax-9*SCALE,top+112*SCALE),(ax+9*SCALE,top+112*SCALE)], fill=COLORS["cyan"]+(235,))
        draw.line((bx, bottom-35*SCALE, bx, bottom-120*SCALE), fill=COLORS["cyan"]+(235,), width=max(1,int(4*SCALE)))
        draw.polygon([(bx,bottom-120*SCALE),(bx-9*SCALE,bottom-102*SCALE),(bx+9*SCALE,bottom-102*SCALE)], fill=COLORS["cyan"]+(235,))
        # Earthquake pulse at the fault.
        pulse = 0.5 + 0.5*math.sin(t*6.0)
        qy = top + 250*SCALE
        for k in range(3):
            r = (18 + k*22 + pulse*18)*SCALE
            draw.ellipse((fault_x-r,qy-r,fault_x+r,qy+r), outline=COLORS["lava"]+(max(30,180-k*45),), width=max(1,int(3*SCALE)))
        draw_text(image, "TRANSFORM MOTION", (OUT_W//2, int(OUT_H*0.292)), size=20 if not QUICK_MODE else 10, fill=COLORS["cyan"]+(240,), bold=True, anchor="ma", stroke=1)
        draw_text(image, "plates slide past one another", (OUT_W//2, int(OUT_H*0.61)), size=15 if not QUICK_MODE else 7, fill=COLORS["white"]+(215,), anchor="ma", stroke=1)
        self.panel(image,(int(OUT_W*0.07),int(OUT_H*0.685),int(OUT_W*0.93),int(OUT_H*0.82)),alpha=184)
        draw_text(image,"MOTION DOES NOT ALWAYS MAKE VOLCANOES",(OUT_W//2,int(OUT_H*0.728)),size=22 if not QUICK_MODE else 11,fill=COLORS["gold"]+(245,),bold=True,anchor="ma",stroke=1)
        draw_text(image,"locked faults can store stress, then slip as earthquakes",(OUT_W//2,int(OUT_H*0.775)),size=16 if not QUICK_MODE else 8,fill=COLORS["white"]+(225,),anchor="ma",stroke=1)

    def draw_hotspot_scene(self, image: Image.Image, t: float):
        shot = next(item for item in SHOT_PLAN if item["name"] == "hotspot")
        local = smoothstep((t - shot["start"]) / max(shot["end"] - shot["start"], 1e-9))
        self.draw_world_map(image, grid_alpha=18)
        self.draw_ring_of_fire(image, 1.0)
        self.draw_boundaries(image, reveal=1.0)
        self.draw_motion_vectors(image, t, alpha=215)
        self.draw_volcanoes(image, reveal=1.0, pulse=t, alpha=220)
        # Moving pulses crawl along the schematic Pacific margins.
        overlay = Image.new("RGBA", OUT_SIZE, (0,0,0,0)); d=ImageDraw.Draw(overlay)
        paths = self.boundary_paths()["subduction"][:2]
        for path_i,path in enumerate(paths):
            pts=self.project_poly(path)
            for j in range(10):
                u=(local*0.8 + j/10 + path_i*0.17) % 1.0
                idx=min(len(pts)-1,int(u*(len(pts)-1)))
                x,y=pts[idx]; r=(5+7*math.sin((u+t)*math.pi)**2)*SCALE
                d.ellipse((x-r,y-r,x+r,y+r),fill=COLORS["lava"]+(170,))
        overlay=overlay.filter(ImageFilter.GaussianBlur(max(1,int(4*SCALE)))); image.alpha_composite(overlay)
        self.panel(image,(int(OUT_W*0.07),int(OUT_H*0.675),int(OUT_W*0.93),int(OUT_H*0.83)),alpha=188)
        draw_text(image,"THE WHOLE MARGIN KEEPS CHANGING",(OUT_W//2,int(OUT_H*0.716)),size=25 if not QUICK_MODE else 12,fill=COLORS["lava"]+(248,),bold=True,anchor="ma",stroke=1)
        draw_text(image,"trenches migrate • plates are consumed • crust is built",(OUT_W//2,int(OUT_H*0.762)),size=16 if not QUICK_MODE else 8,fill=COLORS["white"]+(225,),anchor="ma",stroke=1)
        draw_text(image,"slow motion accumulates into geologic change",(OUT_W//2,int(OUT_H*0.798)),size=13 if not QUICK_MODE else 6,fill=COLORS["muted"]+(190,),anchor="ma",stroke=1)
    ...
    def draw_finale_scene(self, image: Image.Image, t: float):
        ...
        self.draw_world_map(image, grid_alpha=18)
        self.draw_boundaries(image, reveal=1.0)
        self.draw_ring_of_fire(image, 0.85 + 0.15*local)
        self.draw_motion_vectors(image, t, alpha=210)
        self.draw_volcanoes(image, reveal=1.0, pulse=t, alpha=225)
        self.panel(image,(int(OUT_W*0.06),int(OUT_H*0.65),int(OUT_W*0.94),int(OUT_H*0.845)),alpha=195)
        draw_text(image,"THE RING OF FIRE",(OUT_W//2,int(OUT_H*0.695)),size=31 if not QUICK_MODE else 15,fill=COLORS["white"]+(250,),bold=True,anchor="ma",stroke=1)
        draw_text(image,"IS ALWAYS MOVING",(OUT_W//2,int(OUT_H*0.744)),size=38 if not QUICK_MODE else 19,fill=COLORS["lava"]+(250,),bold=True,anchor="ma",stroke=1)
        draw_text(image,"SUBDUCTION",(int(OUT_W*0.25),int(OUT_H*0.797)),size=16 if not QUICK_MODE else 8,fill=COLORS["gold"]+(235,),bold=True,anchor="ma",stroke=1)
        draw_text(image,"FAULT SLIP",(int(OUT_W*0.50),int(OUT_H*0.797)),size=16 if not QUICK_MODE else 8,fill=COLORS["cyan"]+(235,),bold=True,anchor="ma",stroke=1)
        draw_text(image,"VOLCANIC ARCS",(int(OUT_W*0.75),int(OUT_H*0.797)),size=16 if not QUICK_MODE else 8,fill=COLORS["lava"]+(235,),bold=True,anchor="ma",stroke=1)
        draw_text(image,"slow enough to miss — powerful enough to reshape an ocean",(OUT_W//2,int(OUT_H*0.829)),size=14 if not QUICK_MODE else 7,fill=COLORS["white"]+(215,),anchor="ma",stroke=1)
    ...
    def draw_titles(self, image: Image.Image, t: float, shot_name: str):
        ...
        if t < intro_end:
            ...
            draw_text(image,"THE RING OF FIRE",(OUT_W//2,int(OUT_H*0.085)),size=47 if not QUICK_MODE else 23,fill=COLORS["gold"]+(int(250*fade),),bold=True,anchor="ma",stroke=2)
            draw_text(image,"IS ALWAYS MOVING",(OUT_W//2,int(OUT_H*0.135)),size=38 if not QUICK_MODE else 19,fill=COLORS["lava"]+(int(250*fade),),bold=True,anchor="ma",stroke=2)
        labels={
            "pattern":"1 // THE RING",
            "boundaries":"2 // PLATE MOTION",
            "subduction":"3 // SUBDUCTION",
            "rift":"4 // TRANSFORM FAULTS",
            "hotspot":"5 // A CHANGING MARGIN",
            "finale":"6 // ALWAYS MOVING",
        }
    ...
    def draw_source_hud(self, image: Image.Image):
        draw_text(image,"SCHEMATIC TECTONICS VISUAL",(OUT_W-(46 if not QUICK_MODE else 23),62 if not QUICK_MODE else 31),size=15 if not QUICK_MODE else 7,fill=COLORS["orange"]+(225,),bold=True,anchor="ra",stroke=1)
        draw_text(image,f"PACIFIC VOLCANO MARKERS // {len(self.volcanoes)}",(OUT_W-(46 if not QUICK_MODE else 23),91 if not QUICK_MODE else 45),size=14 if not QUICK_MODE else 7,fill=COLORS["muted"]+(195,),anchor="ra",stroke=1)
        draw_text(image,"MOTION VECTORS // SCHEMATIC",(OUT_W-(46 if not QUICK_MODE else 23),117 if not QUICK_MODE else 58),size=13 if not QUICK_MODE else 6,fill=COLORS["muted"]+(175,),anchor="ra",stroke=1)
...
def render_video(scene: RingOfFireScene) -> Path:
    fps = int(CONFIG["fps"])
...
def save_summary() -> Path:
    path = OUTPUT_ROOT / "ring_of_fire_short_summary.json"
    path.write_text(json.dumps({
        ...
        "pacific_volcano_marker_count": 47,
        "science_scope": "schematic explanation of Pacific-margin plate motion and the Ring of Fire",
        "motion_vectors": "illustrative directions only; not live GPS or a quantitative plate-motion model",
        "plate_boundaries": "schematic, not a formal GIS tectonic-boundary dataset",
        "not_a_live_feed": True,
        "story": [
            "Ring of Fire global pattern",
            "continuous relative plate motion",
            "subduction, trenches and volcanic arcs",
            "transform fault motion and earthquakes",
            "long-term deformation of Pacific margins",
            "final synthesis: the Ring of Fire is always moving",
        ],
