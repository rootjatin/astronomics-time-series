...
constantly moving through space, even when the night sky appears frozen. The highlighted systems use
...
YOUTUBE_TITLE = "Stars Are Moving Around Us 🌌 | The Sky Is Not Frozen #Shorts"

YOUTUBE_DESCRIPTION = """The stars may look fixed, but they are constantly moving through space.

This 3D animation visualises the proper motion of nearby stars around our Sun.
...
CONFIG = {
    ...
    "output_basename": "stars_are_moving_around_us",
    "title": "STARS ARE MOVING AROUND US",
    "subtitle": "The night sky is changing every second",
...
FULL_CAPTIONS = [
    (0.5, 7.2, "The stars look fixed, but every one of them is moving through the Milky Way."),
    (7.3, 17.8, "Nearby stars reveal that motion most clearly because their positions shift faster across our sky."),
    (17.9, 29.0, "Astronomers call this proper motion: the slow change in a star's apparent direction over time."),
    (29.1, 40.0, "Barnard's Star has one of the largest known proper motions, crossing the sky faster than most stars."),
    (40.1, 50.0, "These arrows exaggerate the motion so you can see the stellar neighbourhood flowing around the Sun."),
    (50.1, 57.3, "Wait long enough, and familiar constellations disappear as the stars continue on their Galactic orbits."),
]
if QUICK_MODE:
...
class NearbyStarsScene:
    ...
    def draw_intro(self, image: Image.Image, t: float):
        ...
        self.draw_star_map(image, t, yaw, pitch, zoom, (OUT_W * 0.5, OUT_H * 0.43), reveal=progress, label_count=2, radius_limit=6.2, show_background=False, show_motion=True)
        draw_text(image, "NOTHING UP THERE IS STILL", (OUT_W // 2, int(OUT_H * 0.67)), size=ui(62), fill=(245, 249, 255, 245), bold=True, anchor="ma", stroke=max(1, int(3 * SCALE)))
        draw_text(image, "nearby stars are racing through space", (OUT_W // 2, int(OUT_H * 0.725)), size=ui(28), fill=(110, 230, 248, 235), bold=True, anchor="ma", stroke=1)
    ...
    def draw_nearest_cards(self, image: Image.Image, t: float):
        ...
        zoom = 31 * SCALE
        self.draw_star_map(image, t, yaw, pitch, zoom, (OUT_W * 0.5, OUT_H * 0.39), reveal=1.0, label_count=5, radius_limit=8.2, show_background=False, show_motion=True)
    ...
    def draw_full_map(self, image: Image.Image, t: float):
        ...
        self.draw_star_map(image, t, yaw, pitch, zoom, (OUT_W * 0.5, OUT_H * 0.46), reveal=smoothstep(frac * 1.7), label_count=4, radius_limit=12.2, show_background=True, show_motion=True)
        draw_text(image, "A MOVING 3D STAR MAP", (OUT_W // 2, int(OUT_H * 0.73)), size=ui(35), fill=(110, 232, 248, 238), bold=True, anchor="ma", stroke=1)
        draw_text(image, "arrows reveal motion across our sky", (OUT_W // 2, int(OUT_H * 0.77)), size=ui(22), fill=(235, 243, 250, 220), anchor="ma", stroke=1)
    ...
    def draw_outro(self, image: Image.Image, t: float):
        ...
        self.draw_star_map(image, t, yaw, pitch, zoom, (OUT_W * 0.5, OUT_H * 0.39), reveal=1.0, label_count=4, radius_limit=12.2, show_background=True, show_motion=True)
        ...
        image.alpha_composite(panel)
        draw_text(image, "THE SKY OF THE FUTURE", (OUT_W // 2, y0 + int(30 * SCALE)), size=ui(31), fill=(110, 232, 248, 238), bold=True, anchor="ma", stroke=1)
        ...
        draw_text(image, "Thousands of years from now, constellations will look different", (OUT_W // 2, y0 + int(104 * SCALE)), size=ui(21), fill=(255, 183, 105, 228), anchor="ma", stroke=1)
    ...
    def draw_titles(self, image: Image.Image, t: float, shot_name: str):
        ...
        if alpha > 4:
            draw_text(image, "STARS ARE MOVING", (int(56 * SCALE), int(88 * SCALE)), size=ui(52), fill=(245, 249, 253, alpha), bold=True)
            draw_text(image, "AROUND US", (int(56 * SCALE), int(138 * SCALE)), size=ui(52), fill=(245, 249, 253, alpha), bold=True)
            draw_text(image, CONFIG["subtitle"], (int(58 * SCALE), int(194 * SCALE)), size=ui(21), fill=(110, 232, 248, min(alpha, 230)), bold=True)
        labels = {
            "intro": "THE SKY IS NOT FROZEN",
            "nearest": "EVERY STAR HAS A VELOCITY",
            "map3d": "MOTION THROUGH THREE-DIMENSIONAL SPACE",
            "motion": "PROPER MOTION // THE SKY CHANGES",
            "outro": "A MOVING STELLAR NEIGHBOURHOOD",
        }
