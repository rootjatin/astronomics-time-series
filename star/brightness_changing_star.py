...
Default target
--------------
Delta Scuti (δ Scuti), the prototype of a class of short-period pulsating
stars whose brightness variations occur on timescales of hours. The script
automatically chooses an available TESS sector unless you request a specific one.
...
- A phase-folded profile that stacks many cycles into one repeating few-hour pattern.
- An animated stellar disk that expands and contracts in step with the measured
  dominant brightness pattern.
...
deterministic Delta-Scuti-like layout fixture. The fixture is not real TESS
...
Run final quality
-----------------
    python this_star_changes_brightness_every_few_hours_short.py

Run quick preview
-----------------
    FEW_HOURS_STAR_SHORT_QUICK=1 python this_star_changes_brightness_every_few_hours_short.py

Choose another pulsating star
-----------------------------
    FEW_HOURS_STAR_TARGET="AE UMa" python this_star_changes_brightness_every_few_hours_short.py

Choose a TESS sector
--------------------
    FEW_HOURS_STAR_SECTOR=40 python this_star_changes_brightness_every_few_hours_short.py

Force offline layout testing
----------------------------
    FEW_HOURS_STAR_SHORT_OFFLINE=1 FEW_HOURS_STAR_SHORT_QUICK=1 \
        python this_star_changes_brightness_every_few_hours_short.py
"""
...
QUICK_MODE = os.environ.get("FEW_HOURS_STAR_SHORT_QUICK", "0") == "1"
OFFLINE_MODE = os.environ.get("FEW_HOURS_STAR_SHORT_OFFLINE", "0") == "1"
REFRESH = os.environ.get("FEW_HOURS_STAR_SHORT_REFRESH", "0") == "1"
FETCH_PIXELS = os.environ.get("FEW_HOURS_STAR_FETCH_PIXELS", "1") == "1"

TARGET_QUERY = os.environ.get("FEW_HOURS_STAR_TARGET", "Delta Scuti")
SECTOR_TEXT = os.environ.get("FEW_HOURS_STAR_SECTOR", "any").strip()
REQUESTED_SECTOR = int(SECTOR_TEXT) if SECTOR_TEXT and SECTOR_TEXT.lower() not in {"any", "latest", "none"} else None

OUTPUT_ROOT = Path("this_star_changes_brightness_every_few_hours_output")
...
CONFIG = {
    ...
    "basename": "this_star_changes_brightness_every_few_hours",
    "title": "THIS STAR CHANGES",
    "subtitle": "BRIGHTNESS EVERY FEW HOURS • REAL TESS DATA",
    "stars": 580 if QUICK_MODE else 1050,
    "max_render_points": 4200 if QUICK_MODE else 11000,
    "max_saved_points": 100000,
    "period_min_days": 0.025,
...
def make_offline_fixture() -> Tuple[np.ndarray, np.ndarray, TESSMetadata, np.ndarray, np.ndarray]:
    ...
    cadence_s = 120.0
    period_days = 0.19377  # 4.6505 hours
    secondary_period_days = 0.18740
    time_day = np.arange(0, 26.8, cadence_s / 86400.0)
    phase = (time_day / period_days) % 1.0
    phase_secondary = (time_day / secondary_period_days) % 1.0

    # A deterministic Delta-Scuti-like multi-periodic waveform. The primary
    # mode is intentionally strongest so the period finder recovers a cycle of
    # a few hours, while the secondary mode produces gentle beating.
    pulse = (
        0.045 * np.sin(2 * np.pi * phase + 0.22)
        + 0.016 * np.sin(4 * np.pi * phase + 1.05)
        + 0.007 * np.sin(6 * np.pi * phase + 1.75)
        + 0.010 * np.sin(2 * np.pi * phase_secondary + 2.10)
    )
    slow_baseline = 0.0018 * np.sin(2 * np.pi * time_day / 7.8)
    noise = rng.normal(0, 0.0012, len(time_day))
    flux = 1.0 + pulse + slow_baseline + noise
    ...
    metadata = TESSMetadata(
        target_query=TARGET_QUERY,
        object_name="Delta Scuti",
        tic_id="offline fixture",
        sector=REQUESTED_SECTOR or 54,
        camera=1,
        ccd=2,
        author="fixture",
        exposure_s=cadence_s,
        flux_origin="synthetic Delta-Scuti-like fixture flux",
        ra_deg=np.nan,
        dec_deg=np.nan,
        source_kind="offline deterministic few-hours variable-star layout fixture",
        data_status="offline-fixture",
        offline_fixture=True,
        fetched_at_utc=iso_z(utc_now()),
        source_url="https://archive.stsci.edu/missions-and-data/tess",
        note="This fallback resembles a multi-periodic Delta Scuti light curve but is not real TESS photometry.",
    )
...
def collect_data() -> Tuple[TESSMetadata, LightCurveAnalysis, Dict]:
    ...
    summary = {
        ...
        "dominant_period_hours": analysis.dominant_period_days * 24.0 if np.isfinite(analysis.dominant_period_days) else np.nan,
        ...
        "errors": errors,
        "warning": "The dominant period and phase-folded profile are measured from the selected light curve; real short-period pulsators can contain multiple modes, and the expanding-star animation is illustrative.",
    }
...
def save_data(metadata: TESSMetadata, analysis: LightCurveAnalysis, summary: Dict) -> Tuple[Path, Path]:
    ...
    csv_path = DATA_ROOT / "few_hours_star_tess_light_curve.csv"
    json_path = DATA_ROOT / "few_hours_star_summary.json"
...
class PulsatingStarScene:
    ...
    def draw_title(self, img: Image.Image, t: float):
        ...
        shot_titles = {
            ...
            "events": "INSIDE A FEW-HOUR BRIGHTNESS CYCLE",
            "period": "THE DOMINANT REPEATING TIMESCALE",
            "stats": "THE RAPID VARIATION BY THE NUMBERS",
            "outro": "ONE STAR • HUNDREDS OF RAPID CYCLES",
        }
    ...
    def draw_intro(self, img: Image.Image, t: float):
        ...
        # A stylized brightness trace synchronized to the phase-folded data.
        ...
        period_text = f"DOMINANT CYCLE {period * 24.0:.2f} HOURS" if np.isfinite(period) else "DOMINANT CYCLE NOT RESOLVED"
    ...
    def draw_events(self, img: Image.Image):
        self.draw_event_panel(img, self.zoom_box_top, self.analysis.strongest_brightening_index,
                              "BRIGHTEST STAGE OF THE CYCLE", COLORS["gold"], half_width_days=max(0.08, self.analysis.dominant_period_days * 0.55 if np.isfinite(self.analysis.dominant_period_days) else 0.22))
        self.draw_event_panel(img, self.zoom_box_bottom, self.analysis.strongest_dimming_index,
                              "FAINTEST STAGE OF THE CYCLE", COLORS["violet"], half_width_days=max(0.08, self.analysis.dominant_period_days * 0.55 if np.isfinite(self.analysis.dominant_period_days) else 0.30))
        self.draw_pixel_stamp(img, self.pixel_box)
        draw_wrapped_text(img, "The two panels center on the brightest and faintest stages of the rapid variation. The pixel stamp shows where TESS collected the light.",
    ...
    def draw_period(self, img: Image.Image):
        ...
        img.alpha_composite(overlay)
        period = self.analysis.dominant_period_days
        draw_text(img, "MEASURED DOMINANT PERIOD", (x0 + 18, y0 + 18), size=18 if not QUICK_MODE else 8,
                  fill=COLORS["cyan"] + (220,), bold=True, stroke=1)
        draw_text(img, f"{period * 24.0:.2f} HOURS" if np.isfinite(period) else "NOT RESOLVED", (x1 - 18, y0 + 18),
        ...
        draw_wrapped_text(img, "The light curve is folded on the strongest measured period. Stacking many cycles reveals the dominant few-hour pattern more clearly, even when weaker modes are also present.",
    ...
    def draw_stats(self, img: Image.Image):
        ...
        values = [
            ...
            ("Dominant period", f"{self.analysis.dominant_period_days * 24.0:.2f} h" if np.isfinite(self.analysis.dominant_period_days) else "n/a", COLORS["red"]),
    ...
    def draw_outro(self, img: Image.Image):
        ...
        draw_text(img, f"{self.analysis.point_count:,} MEASUREMENTS • {self.analysis.span_days / self.analysis.dominant_period_days:.0f} CYCLES" if np.isfinite(self.analysis.dominant_period_days) else f"{self.analysis.point_count:,} BRIGHTNESS MEASUREMENTS", (W // 2, y0 + 125),
        ...
        draw_wrapped_text(img, "Run the script on another short-period variable or TESS sector and a different few-hour brightness pattern will appear.",
...
def render_video(scene: PulsatingStarScene) -> Path:
    ...
    with iio.get_writer(raw_path, fps=CONFIG["fps"], codec="libx264", quality=8, pixelformat="yuv420p", macro_block_size=None) as writer:
        for frame_index in tqdm(range(frame_count), desc="Rendering few-hours star short"):
